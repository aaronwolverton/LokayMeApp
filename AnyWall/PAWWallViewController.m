...
- (void)viewDidLoad {
    ...
    //self.wallPostsTableViewController.view.frame = CGRectMake(<#CGFloat x#>, <#CGFloat y#>, <#CGFloat width#>, <#CGFloat height#>));
	
    self.wallPostsTableViewController.view.frame = CGRectMake(6.0f, 275.0f, 308.0f, self.view.bounds.size.height - 280.0f);
	...
	*/
	...
	self.navigationItem.titleView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"LokayLogo.png"]];
...
- (MKOverlayView *)mapView:(MKMapView *)mapView viewForOverlay:(id <MKOverlay>)overlay {
	...
	if (version >= 5.1f && [overlay isKindOfClass:[PAWSearchRadius class]]) {
		...
		[(MKOverlayPathView *)result setFillColor:[[UIColor lightGrayColor] colorWithAlphaComponent:0.4f]];
		[(MKOverlayPathView *)result setStrokeColor:[[UIColor darkGrayColor] colorWithAlphaComponent:0.4f]];
...
{
	PAWSettingsViewController *settingsDetailViewController = [[PAWSettingsViewController alloc] initWithNibName:@"PAWSettingsViewController" bundle:nil];
	settingsDetailViewController.modalTransitionStyle = UIModalTransitionStyleCoverVertical;
	
    [self.navigationController presentViewController:settingsDetailViewController animated:YES completion:nil];
